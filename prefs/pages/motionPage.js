...
import { page, group, spinRow, toggleExpander } from '../widgets/rows.js';
...
export function buildMotionPage(window, s) {
    ...
    const genieToggle = toggleExpander(s, 'enable-genie-effect', _('Genie effect'),
        _('Minimize and restore animate into the dock icon'), 'applications-graphics-symbolic');
    genieToggle.add_row(spinRow(s, 'genie-duration', _('Genie duration'),
        _('Length of the minimize animation (milliseconds)'), 50, 1000, 10, 0));
    genie.add(genieToggle);
    p.add(genie);
}
